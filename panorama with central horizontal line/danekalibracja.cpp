...
int get_ocam_model(ocam_model *myocam_model, char *filename)
{
...
//------------------------------------------------------------------------------
void cam2world(double point3D[3], double point2D[2], ocam_model *myocam_model)
{
	double *pol = myocam_model->pol;
	double xc = (myocam_model->xc);
	double yc = (myocam_model->yc);
	...
	Opoint2d2vu(pointVU , point2D , myocam_model);
        double xp = pointVU[0];
        double yp = pointVU[1];
...
void world2cam(double point2D[2], double point3D[3], ocam_model *myocam_model)
{
	double *invpol = myocam_model->invpol;
	double xc = (myocam_model->xc);
	double yc = (myocam_model->yc);
	...
	if (norm != 0)
	{
		...
		//point2D[1] = x*e + y + yc;
		Ovu2point2d(point2D , pointVU , myocam_model);
	}
	else
	{
		point2D[0] = xc;
...
void create_panoramic_undistortion_LUT(Mat mapx, Mat mapy, float Rmin, float Rmax, float xc, float yc)
{
	...
	for (i = 0; i<height; i++)
		for (j = 0; j<width; j++)
		{
			theta = -((float)j) / width * 2 * PI; // Note, if you would like to flip the image, just inverte the sign of theta
...
{  
	...
	for(j = 0 ; j < width ; j++)
	{
		...
		theta = -((float)j) / width * 2 * PI; // kat theta dla danej kolumny na panoramie 
...
void Ocreate_projection( Mat mapx, Mat mapy, double f, double fi, double theta, ocam_model *myocam_model)
{
	...
	int height = mapx.rows; // wysokosc obrazu
        ...
        double Rtan = 0;
        ///////////////////////////////////////////
        double Rtheta = (theta/360.0)*2*PI;
        double Rfi = (fi/360.0)*2*PI;
